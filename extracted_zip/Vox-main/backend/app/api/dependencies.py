...
# We'll keep oauth2_scheme for Swagger UI documentation if needed, but our actual logic uses cookies.
...
def get_token(request: Request, token: Optional[str] = Depends(oauth2_scheme)) -> str:
    ...
    cookie_token = request.cookies.get("vox_token")
    if cookie_token:
        return cookie_token
    if token:
        return token
...
def get_conversation_service(
    ...
    return ConversationService(db, ai_service, contact_service, email_service)
